...
void Zumo32U4ModulesMotors::motorDrive(int left, int right) {
  Zumo32U4Motors::setRightSpeed(right); // Alternatively, run Zumo32U4ModulesMotors::setSpeeds(left, right);
  Zumo32U4Motors::setLeftSpeed(left); // This function executes setLeftSpeed(int) & setRightSpeed(int) one after another
}

void Zumo32U4ModulesMotors::motorFlip() {
  Zumo32U4ModulesMotors::reverse = !Zumo32U4ModulesMotors::reverse; // Change bool
  Zumo32U4Motors::flipLeftMotor(Zumo32U4ModulesMotors::reverse); // Update left motor direction in accordance to bool
  Zumo32U4Motors::flipRightMotor(Zumo32U4ModulesMotors::reverse); // Update right motor direction likewise
}

void Zumo32U4ModulesEncoders::getMotorDistance() {
  motorDistance[0] = 8 * acos(0.0) * Zumo32U4ModulesEncoders::getCountsLeft() / CPR; // [...] = 8 * acos(0.0) * count / 900... CPR table value: https://www.pololu.com/docs/0J63/3.4
  motorDistance[1] = 8 * acos(0.0) * Zumo32U4ModulesEncoders::getCountsRight() / CPR; // If distance does not measure up, adjust using class variable CPR.
}

void Zumo32U4ModulesEncoders::getMotorDistanceReset() {
  motorDistance[0] = 8 * acos(0.0) * Zumo32U4ModulesEncoders::getCountsAndResetLeft() / CPR; // Basically getMotorDistance() where encoders, measured distances, are reset
  motorDistance[1] = 8 * acos(0.0) * Zumo32U4ModulesEncoders::getCountsAndResetRight() / CPR;
}
...
void Zumo32U4ModulesLineSensors::getLineSensorValue() { Zumo32U4LineSensors::read(lineSensorValue, QTR_EMITTERS_ON); }

void Zumo32U4ModulesLineSensors::calibrateLineSensor() { Zumo32U4LineSensors::calibrate(QTR_EMITTERS_ON); }

void Zumo32U4ModulesLineSensors::getLineSensorValueCalibrated() { Zumo32U4LineSensors::readCalibrated(lineSensorValue, QTR_EMITTERS_ON); }
...
void Zumo32U4Modules::PIDLineFollower(float kp, float ki, float kd, int speed, bool fiveLineSensors, bool dir, bool blackLine=false) {
  int integral = 0;
  int error = 0;
  int lastError = 0;
  ...
  while (proximity != 12) { // Run PID-controller while both proximity sensors both do not read max value
    ...
    else if (lineSensorValue[4] < 300) { // If right side lineSensor value is low:
      Zumo32U4ModulesMotors::setSpeeds(200, -60); // Turn right by 90 degrees (roughly speaking)
      delay(650);
    }*/
    /*if (lineSensorValue[2] > 250 && lineSensorValue[1] > 300 && lineSensorValue[3] > 300) { // If mid lineSensor is not in white
      Zumo32U4Motors::setSpeeds(-200, 200); // Turn left by 180 degrees (roughly speaking)
      delay(650);
    }*/
    if (fiveLineSensors == true && dir==true && blackLine==true) { error = lineSensorValue[0] + lineSensorValue[1] - lineSensorValue[2] - lineSensorValue[3]; }
    else if (fiveLineSensors == true && dir==false && blackLine==true) { error = lineSensorValue[1] + lineSensorValue[2] - lineSensorValue[3] - lineSensorValue[4]; }
    else if (fiveLineSensors == true && dir==true && blackLine==false) { error = lineSensorValue[3] - lineSensorValue[1]; } // lineSensorValue[4] +  - lineSensorValue[2] 
    else if (fiveLineSensors == true && dir==false && blackLine==false) { error = lineSensorValue[3] - lineSensorValue[1] ; } //   + lineSensorValue[2]  - lineSensorValue[0]
    else if (fiveLineSensors == false && blackLine==true) { error =  lineSensorValue[0] - lineSensorValue[4]; } //  + lineSensorValue[2]
    else if (fiveLineSensors == false && blackLine==false) { error = lineSensorValue[4] - lineSensorValue[0]; } // - lineSensorValue[2]
    
    /*else if (LCD == false && dir==true && blackLine==true) { error = lineSensorValue[0] - lineSensorValue[4]; } //  + lineSensorValue[2]
    else if (LCD == false && dir==false && blackLine==true) { error =  lineSensorValue[4] + lineSensorValue[2] - 2*lineSensorValue[0]; } // - lineSensorValue[2]
    else if (LCD == false && dir==true && blackLine==false) { error =  lineSensorValue[4] - lineSensorValue[2] - lineSensorValue[0]; } //  + lineSensorValue[2]
    else if (LCD == false && dir==false && blackLine==false) { error = 2*lineSensorValue[4] - lineSensorValue[2] - lineSensorValue[0]; } // - lineSensorValue[2]*/
     // Ideally, the Zumo32U4 will drive in between the (white) line // lineSensorValue[0]*1.15+ -lineSensorValue[4] //1.2, 0, 0.8, 200 || 0.9, 0, 0.15, 100
    int pFix = error * kp;
    int iFix = integral * ki;
    int dFix = (error - lastError) * kd; // Where derivative = (error - lastError)
    int correction = pFix + iFix + dFix;
    integral += error; // Let integral accumulate based on error
